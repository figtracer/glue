...
The main-branch policy is checked in at [branch-protection.json](../.github/branch-protection.json): an up-to-date passing `CI` check, pull requests with no mandatory approvals, resolved conversations, linear history and no force pushes or deletion. It includes admins.
...
Maintainers can merge their own PRs once required checks pass and conversations are resolved. Reviews and CODEOWNERS routing are optional. Do not disable protection to merge failing checks.
