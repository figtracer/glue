...
Actions are restricted to GitHub-owned actions with full SHA pins. Workflow tokens are read-only and cannot approve PRs. Dependabot checks development dependencies and Actions weekly. Review Action release dates before merging updates; pins should be at least seven days old unless a security fix warrants an exception.
