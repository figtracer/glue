...
`glue init refill` configures a local service for one wallet, a fleet, prepared work, or a Tempo token balance. For native gas, it uses the existing Glue MPP API on Base, Ethereum, Arbitrum and Optimism. launchd or a systemd user timer wakes the worker; Glue checks whether action is needed, pays once and reconciles delivery.
...
Refill now covers single wallets, fleets, prepared work and Tempo token balances; see the [service catalog](services/README.md). More services should earn their place through real usage; we do not need a generic plugin runtime yet.
